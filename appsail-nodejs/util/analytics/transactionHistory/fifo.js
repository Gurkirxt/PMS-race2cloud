 ...
 * SPLIT must be processed BEFORE BONUS so that bonus shares (which are stored
 * in the DB as the post-split count) are not multiplied a second time by the
 * split. Without this, an account holding 7 BUY shares with a same-date 56-share
 * bonus and a 1:2 split would end up at 126 shares instead of the correct 70.
 */
 ...
 * (Math.floor / ratio logic) is intentional and unaffected.
 */
const QTY_EPS = 1e-6;
const snapQty = (n) => {
  const v = Number(n) || 0;
